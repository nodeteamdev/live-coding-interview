...
(function (mod) {
    ...
    CodeMirror.defineMode('javascript', (config, parserConfig) => {
        ...
        const { statementIndent } = parserConfig;
        ...
        const keywords = (function () {
            ...
            const A = kw('keyword a'); const B = kw('keyword b'); const C = kw('keyword c'); const
                D = kw('keyword d');
        ...
        function expect(wanted) {
            function exp(type) {
                ...
                if (wanted == ';' || type == '}' || type == ')' || type == ']') return pass();
                return cont(exp);
            }
            return exp;
        }
        ...
        function maybeTarget(noComma) {
            return function (type) {
                ...
                if (type == 'variable' && isTS) return cont(maybeTypeArgs, noComma ? maybeoperatorNoComma : maybeoperatorComma);
        ...
        function classfield(type, value) {
            ...
            if (value == '=') return cont(expressionNoComma);
            const context = cx.state.lexical.prev; const
      ...
      || (state.lastType == 'quasi' && /\{\s*$/.test(stream.string.slice(0, stream.pos - (backUp || 0))));
        ...
        return {
            ...
            indent(state, textAfter) {
                ...
                const firstChar = textAfter && textAfter.charAt(0); let { lexical } = state; let
                    top;
             ...
             && (firstChar == '}' || ((top = state.cc[state.cc.length - 1])
                ...
                if (type == 'stat') return lexical.indented + (isContinuedStatement(state, textAfter) ? statementIndent || indentUnit : 0);
                if (lexical.info == 'switch' && !closing && parserConfig.doubleIndentSwitch != false) return lexical.indented + (/^(?:case|default)\b/.test(textAfter) ? indentUnit : 2 * indentUnit);
