...
(function (mod) {
    ...
    CodeMirror.defineMode('css', (config, parserConfig) => {
        const { inline } = parserConfig;
        if (!parserConfig.propertyKeywords) parserConfig = CodeMirror.resolveMode('text/css');

        const { indentUnit } = config;
        const { tokenHooks } = parserConfig;
        ...
        const { allowNested } = parserConfig;
        const { lineComment } = parserConfig;
        ...
        states.restricted_atBlock = function (type, stream, state) {
          ...
          || (state.stateArg == '@counter-style' && !counterDescriptors.hasOwnProperty(stream.current().toLowerCase()))) override = 'error';
