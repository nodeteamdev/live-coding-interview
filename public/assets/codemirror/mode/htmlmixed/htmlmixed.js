...
(function (mod) {
    ...
    function maybeBackup(stream, pat, style) {
        const cur = stream.current(); const
    ...
    CodeMirror.defineMode('htmlmixed', (config, parserConfig) => {
        ...
        const tags = {};
        const configTags = parserConfig && parserConfig.tags; const
            configScript = parserConfig && parserConfig.scriptTypes;
        addTags(defaultTags, tags);
        if (configTags) addTags(configTags, tags);
        if (configScript) { for (let i = configScript.length - 1; i >= 0; i--) tags.script.unshift(['type', configScript[i].matches, configScript[i].mode]); }

        function html(stream, state) {
            const style = htmlMode.token(stream, state.htmlState); const tag = /\btag\b/.test(style); let
                tagName;
          ...
          && tags.hasOwnProperty(tagName)) {
                ...
                const endTagA = getTagRegexp(inTag[1], true); const
        ...
        return {
            startState() {
                ...
                return {
                    token: html, inTag: null, localMode: null, localState: null, htmlState: state,
                };
            },
